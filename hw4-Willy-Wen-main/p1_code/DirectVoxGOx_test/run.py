...
def config_parser():
    ...
    parser.add_argument("--no_reload_optimizer", action='store_true', default=False,
...
def render_viewpoints(model, render_poses, HW, Ks, ndc, render_kwargs,
    ...
    '''
    print('len',len(render_poses),len(HW),len(Ks))
...
def _compute_bbox_by_cam_frustrm_bounded(cfg, HW, Ks, poses, i_test, near, far):
    xyz_min = torch.Tensor([np.inf, np.inf, np.inf])
    xyz_max = -xyz_min
    for (H, W), K, c2w in zip(HW[i_test], Ks[i_test], poses[i_test]):
        rays_o, rays_d, viewdirs = dvgo.get_rays_of_a_view(
                H=H, W=W, K=K, c2w=c2w,
...
def _compute_bbox_by_cam_frustrm_unbounded(cfg, HW, Ks, poses, i_test, near_clip):
...
def compute_bbox_by_cam_frustrm(args, cfg, HW, Ks, poses, i_test, near, far, **kwargs):
    print('compute_bbox_by_cam_frustrm: start')
    if cfg.data.unbounded_inward:
        xyz_min, xyz_max = _compute_bbox_by_cam_frustrm_unbounded(
                cfg, HW, Ks, poses, i_test, kwargs.get('near_clip', None))

    else:
        xyz_min, xyz_max = _compute_bbox_by_cam_frustrm_bounded(
                cfg, HW, Ks, poses, i_test, near, far)
...
def scene_rep_reconstruction(args, cfg, cfg_model, cfg_train, xyz_min, xyz_max, data_dict, stage, coarse_ckpt_path=None):
    ...
    if reload_ckpt_path is None:
        ...
        if cfg_model.maskout_near_cam_vox:
            model.maskout_near_cam_vox(poses[i_test,:3,3], near)
    else:
...
if __name__=='__main__':
    ...
    if args.render_test:# or args.render_train: # or args.render_video:
        if args.ft_path:
            ...
            ckpt_path = os.path.join('fine_last.tar')
    ...
    if args.render_test:
        testsavedir = os.path.join(args.output)
        os.makedirs(testsavedir, exist_ok=True)
        print('All results are dumped into', testsavedir)
        rgbs, depths, bgmaps = render_viewpoints(
