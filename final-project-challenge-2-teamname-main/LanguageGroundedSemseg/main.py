...
def preprocess_dataset(config):
    # 錄製 train, train.txt 到指定的資料夾
    # 從 train 中 sample 10 個資料到 val 並產生 val.txt 的資料
    if 'val' not in os.listdir(config.scannet_path):
        src = config.ply_path
        dst = config.scannet_path
        shutil.copyfile(src, dst)
        src = config.txt_path
        dst = config.scannet_path
        shutil.copyfile(src, dst)
        os.makedirs(os.path.join(config.scannet_path, 'val'), exist_ok=True)
        for idx, name in enumerate(os.listdir(config.ply_path)):
            if idx > 10:
                break
            shutil.copyfile(os.path.join(config.ply_path, name), os.path.join(config.scannet_path, 'val', name))
            with open(os.path.join(config.scannet_path, 'val.txt'), 'a') as f:
                f.write(f'val/{name}\n')
        print('>>>>>Finish createing dataset!')
    else:
        print('>>>>>Skipping create data!')
...
def main():
    ...
    print('>>>>>dataset:', len(data_loader.dataset))
