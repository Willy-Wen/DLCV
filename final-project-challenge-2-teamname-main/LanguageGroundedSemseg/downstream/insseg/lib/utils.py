...
def print_info(iteration,
    ...
    debug_str += "\tAVG Loss {loss:.3f}\t" \
        ...
        loss=losses.detach().item(), top1=scores.item(), mIOU=np.nanmean(ious),
...
def visualize_instances(coords, clustered_results, labels, gt, gt_instances, config, scene_name):


    coords_np = coords[:, 1:].cpu().numpy()
    instance_labels = np.zeros((coords.shape[0]))

    # Create a cloud with the instance ids
    for inst_res_id, inst_res in clustered_results.items():
        inst_mask = inst_res['pred_mask']
        instance_labels[inst_mask == 1] = inst_res_id

    colors = list(SCANNET_COLOR_MAP_200.values())
    color_mapper = lambda id: colors[int(id) % 200 + 1]
    point_colors = np.vectorize(color_mapper)(instance_labels)
    point_colors = np.array(point_colors).T

    colored_cloud = np.concatenate((coords_np, point_colors), axis=1)
    output_name = os.path.join(config.test.visualize_path, f'{scene_name}_rgb_instances.ply')

    # make dir
    if not os.path.exists(os.path.dirname(output_name)):
        os.makedirs(os.path.dirname(output_name))

    # save rgb cloud of instances
    save_point_cloud(colored_cloud, output_name, verbose=False)

    # save semantic cloud
    semantic_cloud = np.concatenate((coords_np, labels.cpu().numpy()[:,None]), axis=1)
    semantic_cloud_output_name = os.path.join(config.test.visualize_path, f'{scene_name}_labels.npy')
    np.save(semantic_cloud_output_name, semantic_cloud)

    # Save gt instances
    gt_instance_ids = gt_instances['ids']
    gt_instance_ids[gt_instance_ids == -1] = 0
    point_colors = np.vectorize(color_mapper)(gt_instance_ids)
    point_colors = np.array(point_colors).T
    colored_cloud = np.concatenate((coords_np, point_colors), axis=1)
    gt_instances_cloud_output_name = os.path.join(config.test.visualize_path, f'{scene_name}_gt_rgb_instances.ply')
    save_point_cloud(colored_cloud, gt_instances_cloud_output_name, verbose=False)

    np_gt_instances_cloud_output_name = os.path.join(config.test.visualize_path, f'{scene_name}_gt_instances.npy')
    np.save(np_gt_instances_cloud_output_name, np.concatenate((coords_np, gt_instance_ids[:, None]), axis=1))


    # save semantic gt
    gt_cloud = np.concatenate((coords_np, gt.cpu().numpy()[:, None]), axis=1)
    gt_cloud_output_name = os.path.join(config.test.visualize_path, f'{scene_name}_gt.npy')
    np.save(gt_cloud_output_name, gt_cloud)
