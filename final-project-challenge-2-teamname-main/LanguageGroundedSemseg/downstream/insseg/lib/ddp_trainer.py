...
from downstream.insseg.lib import test as test_
...
class SegmentationTrainer:
    def __init__(self, config):
        ...
        if config.net.weights != '' and config.net.weights is not None:
            if not os.path.isfile(config.net.weights):
                logging.info('Weight file does not exists: ' + config.net.weights)
            else:
        ...
        if self.is_master:
            ...
            wandb.init(project="3DInsseg", name=run_name, config=OmegaConf.to_container(config, resolve=True))
            # self.writer = SummaryWriter(log_dir='tensorboard', filename_suffix=f'_{wandb.run.id}')
        ...
        checkpoint_fn = config.misc.log_dir + '/' + 'weights/weights.pth'
        ...
        if os.path.isfile(checkpoint_fn):
            ...
            self.curr_iter = state['iteration'] + 16
    ...
    def validate(self):
        ...
        # self.writer.add_scalar('val/mAP_0.5', val_mAP, self.curr_iter)

        wandb_log = {'val/miou': val_miou, 'val/loss': val_loss,
                     'val/precision_at_1': val_score, 'val/mAP@0.5': val_mAP}
        wandb.log(wandb_log)
        ...
        while self.is_training:
            for _ in range(len(self.train_data_loader) // self.config.optimizer.iter_size):
                ...
                for sub_iter in range(self.config.optimizer.iter_size):
                    ...
                    sinput = SparseTensor(input, coords, device=self.cur_device)
                    ...
                    # gather information
                    logging_output = {'total_loss': total_loss.detach().item(), 'semantic_loss': semantic_loss.detach().item(), 'score': score / self.config.optimizer.iter_size}
                    logging_output['offset_dir_loss'] = offset_dir_loss.detach().item()
                    logging_output['offset_norm_loss'] = offset_norm_loss.detach().item()
                ...
                if self.curr_iter % self.config.train.stat_freq == 0 or self.curr_iter == 1:
                    ...
                    if self.is_master:
                        ...
                        wandb_log = {'train/loss': losses['total_loss'].avg,
                                     'train/semantic_loss': losses['semantic_loss'].avg,
                                     'train/offset_dir_loss': losses['offset_dir_loss'].avg,
                                     'train/offset_norm_loss': losses['offset_norm_loss'].avg,
                                     'train/precision_at_1': scores.avg,
