...
def preprocess_data(config):
    if 'val' not in os.listdir(config.scannet_path):
        target_root=config.scannet_path
        os.makedirs(os.path.join(target_root, 'val'), exist_ok=True)
        ply_path = config.ply_path
        txt_path = config.txt_path
        folder_name = ply_path.split('/')[-1]
        for _, file in enumerate(tqdm(os.listdir(ply_path))):
            ori_file_path = os.path.join(ply_path, file)
            with open(ori_file_path, 'rb') as f:
                plydata = PlyData.read(f)
            a = read_plyfile(ori_file_path)
            ...
            with open(os.path.join(target_root, 'val.txt'), 'a') as f:
                f.write(f'val/{file}\n')
        print('>>>>>Finish create dataset!')
    else:
        print('>>>>>Dataset has exist, stop create!')
...
def main():
    ...
    # trainer = Trainer(max_epochs=config.max_epoch, logger=loggers,
    #                   devices=num_devices, accelerator="gpu",
    #                   num_sanity_val_steps=4, accumulate_grad_batches=1,
    #                   callbacks=[*checkpoint_callbacks, CleanCacheCallback()])
    # print('>>>>>Model:', model)
    print('>>>>>dataset:', len(data_loader.dataset))
    print('>>>>>dataset detailed:', data_loader.dataset[0][2], data_loader.dataset[0][3])
