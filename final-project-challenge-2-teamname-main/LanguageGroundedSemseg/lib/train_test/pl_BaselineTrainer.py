...
from lib.utils import nanmean_t, fast_hist_torch, per_class_iu_torch, visualize_results, print_info, loss_by_name, save_feature_maps
...
class BaselineTrainerModule(LightningModule):
    ...
    def validation_step_end(self, outputs):
        ...
        if self.config.visualize_freq == 0 or self.val_iteration % self.config.visualize_freq == 0 and self.val_iteration > 0:
            ...
            if self.config.visualize:
                ...
                # save_feature_maps(feature_maps=outputs['feature_maps'].features, config=self.config, \
                # scene_name=outputs['scene_name'], targets=target, coords=outputs['coords'])
        ...
        if self.val_iteration % self.config.test_stat_freq == 0 and self.val_iteration > 0:
            ...
            print_info(
                ...
                self.losses.compute().detach().item(),
                (self.scores.compute() * 100.).item(),
                ious.cpu().numpy() * 100.,
    ...
    def on_test_epoch_end(self):
        ...
        self.dataset.test_pointcloud(self.config.log_dir, self.config.visualize_path, self.num_labels)
