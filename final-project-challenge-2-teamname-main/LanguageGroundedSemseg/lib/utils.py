...
from lib.losses.UnifiedFocalLoss import unified_loss
...
def loss_by_name(loss_name, ignore_index=0, alpha=0.5, gamma=2.0, reduction='mean', weight=None):
    print(f'>>>>>Now using {loss_name} loss!')
    if loss_name == 'focal':
        ...
        return unified_loss(weight=0.5, delta=0.6, gamma=0.2, ignore_index=ignore_index)
    else:
        return None
...
def visualize_results(coords, colors, target,
    ...
    ptc_nonpred = np.hstack(
        (input_xyz[target_nonpred].cpu().numpy(), np.zeros((np.sum(target_nonpred.cpu().numpy()), 3))))  # type: np.ndarray
    ptc_nonpred_np = np.hstack(
        (input_xyz[target_nonpred].cpu().numpy(), np.zeros((np.sum(target_nonpred.cpu().numpy()), 1))))  # type: np.ndarray
...
def save_feature_maps(feature_maps, config, scene_name, targets=None, coords=None):
    base_file_name = '_'.join([scene_name[0], "feature_maps.pkl"])
    # Create directory to save visualization results.
    os.makedirs(config.visualize_path, exist_ok=True)
